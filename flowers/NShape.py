...
class NShape:
    def __init__(self, sides, sidelen, colour=None, border=False, borderColour=None, curved=False):
        ...
        if colour == None:
            ...
            self.colour = (random(360), 0.8, 0.8, 0.6)
        else:
            self.colour = colour
        ...
        if borderColour == None:
            ...
            self.borderColour = (random(360), 0.8, 0.8, 0.6)
        else:
    ...
    def drawShape(self, x, y, blur=0, blended=0, bigger=True):
        ...
        if self.curved:
            s.curveVertex(self.vertices[0][0] - minx + pad, self.vertices[0][1] - miny + pad)
        for p in self.vertices:
            if self.curved:
                s.curveVertex(p[0] - minx + pad, p[1] - miny + pad)
            else:
